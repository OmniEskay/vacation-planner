...
function DestinationPage() {
  ...
  const destinationId = parseInt(id, 10); // Ensure ID is an integer
  const [selectedDestination, setSelectedDestination] = useState(null);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState(null);

  // Trip List state and localStorage handling (remains the same)
  ...
  useEffect(() => {
    ...
    const fetchDestination = async () => {
      ...
      try {
        ...
        setSelectedDestination(data); // Set the fetched destination object
      } catch (err) {
  ...
  // Trip List toggle logic (remains the same)
  ...
  if (!selectedDestination) {
     // This case might be covered by the 404 error, but good as a fallback
    return (
      <div className="min-h-screen flex items-center justify-center text-xl text-blue-500">
  ...
  // Use direct image paths (assuming /public folder structure)
  const imageErrorHandler = (e) => {
      e.target.onerror = null; // prevent loops
      e.target.src = "https://via.placeholder.com/400x300"; // Use a reliable placeholder
  };
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mb-5">
              {selectedDestination.images?.map((img, index) => (
                <img
                  ...
                  onError={imageErrorHandler} // Fallback for each image
                  loading="lazy"
                />
              ))}
            </div>
